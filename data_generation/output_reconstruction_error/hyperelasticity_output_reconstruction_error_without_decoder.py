    ...
    if rank == 0:
        ...
        for i,rank in enumerate(rank_array):
            ...
            for j in range(num_functions_for_error_estimation):
                ...
                ASM_error[j,i] = dolfin.errornorm(reconstructed_solution, output_functions[j], norm_type='L2')/dolfin.norm(output_functions[j], norm_type='L2')
    ...
    if rank == 1:
        ...
        for i,rank in enumerate(rank_array):
            ...
            for j in range(num_functions_for_error_estimation):
                ...
                low_rank_input_function.vector().set_local(low_rank_input_nodal_values[j,:])
                reconstructed_solution = hyperelasticity.solve(m=low_rank_input_function)
                KLE_error[j,i] = dolfin.errornorm(reconstructed_solution, output_functions[j], norm_type='L2')/dolfin.norm(output_functions[j], norm_type='L2')
